...
### B-16 Blurry mesh never settles in some areas (ice); storm on P
Austin/Andrew 2026-07-23: around certain areas (ice especially) the
live view stays blurry indefinitely — redraw never converges; pressing
P forces settle AND prints an eviction storm. Photo pose 37.037
122.505 @ 2.62. COLD-STATE REPRO FAILS: harness at that pose is
byte-stable in 3 s (b16-repro2.play), so the thrash needs accumulated
session state. THEORY: frozen-water tiles carry extra geometry; in ice
regions the DRAWN working set can exceed TILE_RETAIN/HARD budgets, so
gentle eviction sheds tiles that are immediately rebuilt — each
