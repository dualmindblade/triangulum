...
// raster and used to be repeated independently by every level-11..14 tile.
// Partition that immutable annual result into disjoint level-14 regions
// (~800 m on Neisor). Boundary-crossing entries refine their rejection proof
// adaptively to level 20 (~12 m) without turning every small proof cell into
// a locked cache entry. The partition is independent of render LOD; a parent
// and all of its descendants address the same entries.
...
/// The annual, edit-independent half of one forest impostor decision.
/// Absolute coordinates and the lottery value are reconstructed from the
/// owning region, keeping the retained stream compact without changing a
/// floating-point comparison in the tile builder.
#[derive(Clone, Copy)]
struct LeanImpostorCandidate {
    density: f64,
    ci_offset: u16,
    cj_offset: u16,
    kind: crate::voxel::TreeKind,
}

const _: () = assert!(std::mem::size_of::<LeanImpostorCandidate>() <= 16);
...
struct ImpostorCandidateRegion {
    /// One disjoint tier per lattice: stride 8, then the stride-4 points not
    /// on 8, and so on. Finer requests reuse every already-built coarser tier.
    tiers: [OnceLock<Arc<[LeanImpostorCandidate]>>; 4],
...
impl ImpostorCandidateRegion {
    fn new(last_use: u64) -> Self {
        Self {
            tiers: std::array::from_fn(|_| OnceLock::new()),
            ...
            || shard.resident_bytes > IMPOSTOR_CANDIDATE_CACHE_BYTES_PER_SHARD
        {
            ...
            let victim = shard
                ...
                .min_by_key(|(key, entry)| {
                    (entry.last_use.load(Ordering::Relaxed), **key)
                })
                .map(|(key, _)| *key);
...
impl Planet {
    ...
    fn impostor_candidate_region_rejects_all(
        &self,
        key: ImpostorCandidateRegionKey,
    ) -> bool {
    ...
    fn retain_impostor_candidate_proof_cell(
        ...
        if depth == 0 {
            cells.push(ImpostorCandidateProofCell {
                ci0: (ci0 - region_ci0) as u16,
                ci1: (ci1 - region_ci0) as u16,
                cj0: (cj0 - region_cj0) as u16,
                cj1: (cj1 - region_cj0) as u16,
            });
            return;
        }
        ...
        for pi in 0..2 {
            let child_ci0 = ci0 + (pi * ci_width).div_ceil(2);
            let child_ci1 = ci0 + ((pi + 1) * ci_width).div_ceil(2) - 1;
            for pj in 0..2 {
                let child_cj0 = cj0 + (pj * cj_width).div_ceil(2);
                let child_cj1 = cj0 + ((pj + 1) * cj_width).div_ceil(2) - 1;
    ...
    fn build_impostor_candidate_proof_cells(
        ...
        for pi in 0..divisions {
            ...
            for pj in 0..divisions {
                ...
                let rejects = self
                    .impostor_candidate_bounds_phase_reject_all(key.face, ci0, ci1, cj0, cj1);
                rejected_any |= rejects;
        ...
        if !rejected_any {
            cells.push(ImpostorCandidateProofCell {
                ci0: 0,
                ci1: (region_ci1 - region_ci0) as u16,
                cj0: 0,
                cj1: (region_cj1 - region_cj0) as u16,
            });
        } else {
    ...
    fn build_impostor_candidate_tier(
        &self,
        key: ImpostorCandidateRegionKey,
        stride: u64,
        proof_cells: &[ImpostorCandidateProofCell],
    ) -> Arc<[LeanImpostorCandidate]> {
        debug_assert!(IMPOSTOR_CANDIDATE_STRIDES.contains(&stride));
        let (ci0, ci1) = Self::impostor_candidate_region_bounds(key.ri);
        let (cj0, cj1) = Self::impostor_candidate_region_bounds(key.rj);
        let nnf = COLUMNS_PER_FACE as f64;
        ...
        let seed = self.seed;
        debug_assert!(ci1 - ci0 < u16::MAX as u64 && cj1 - cj0 < u16::MAX as u64);
        let barren_interior = |koppen, forest| {
            forest <= 1e-4
                && !matches!(
                    crate::voxel::tree_kind_density(koppen),
        ...
        for cell in proof_cells {
            let cell_ci0 = ci0 + u64::from(cell.ci0);
            let cell_ci1 = ci0 + u64::from(cell.ci1);
            let cell_cj0 = cj0 + u64::from(cell.cj0);
            let cell_cj1 = cj0 + u64::from(cell.cj1);
            let ci_start = cell_ci0.div_ceil(stride) * stride;
            let cj_start = cell_cj0.div_ceil(stride) * stride;
            if ci_start > cell_ci1 || cj_start > cell_cj1 {
                continue;
            }
            for ci in (ci_start..=cell_ci1).step_by(stride as usize) {
                for cj in (cj_start..=cell_cj1).step_by(stride as usize) {
                    // Tiers are disjoint: a point belongs to the coarsest
                    // lattice on which it can appear, and is evaluated once.
                    if stride < 8
                        && ci.is_multiple_of(next_stride)
                        && cj.is_multiple_of(next_stride)
                    {
                        continue;
                    }
                    let lot = crate::voxel::tree_hash01(key.face, ci, cj, seed);
                    if lot >= crate::voxel::MAX_TREE_DENSITY * comp {
                        continue;
                    }
                    let u = -1.0 + 2.0 * (ci as f64 + 0.5) / nnf;
                    let v = -1.0 + 2.0 * (cj as f64 + 0.5) / nnf;
                    let interior = self.vegetation_interior(face, u, v);
                    if interior.is_some_and(|(koppen, forest)| barren_interior(koppen, forest)) {
                        continue;
                    }
                    let (temp, precip) = self.temp_precip(face, u, v);
                    ...
                    candidates.push(LeanImpostorCandidate {
                        density,
                        ci_offset: (ci - ci0) as u16,
                        cj_offset: (cj - cj0) as u16,
                        kind,
                    });
                }
            }
    ...
    fn impostor_candidate_region_tiers(
        &self,
        key: ImpostorCandidateRegionKey,
        stride: u64,
    ) -> Vec<Arc<[LeanImpostorCandidate]>> {
        ...
        let proof_cells = entry.proof_cells.get_or_init(|| {
            let proof_cells = self.build_impostor_candidate_proof_cells(key);
            self.impostor_candidates.record_bytes(
                key,
                &entry,
                proof_cells.len() * std::mem::size_of::<ImpostorCandidateProofCell>(),
            );
            proof_cells
        });
        let first = IMPOSTOR_CANDIDATE_STRIDES
            .iter()
            .position(|&tier_stride| tier_stride == stride)
            .expect("fine impostor stride");
        (first..IMPOSTOR_CANDIDATE_STRIDES.len())
            .map(|index| {
                let tier_stride = IMPOSTOR_CANDIDATE_STRIDES[index];
                let candidates = entry.tiers[index].get_or_init(|| {
                    let candidates = self.build_impostor_candidate_tier(
                        key,
                        tier_stride,
                        proof_cells,
                    );
                    self.impostor_candidates.record_bytes(
                        key,
                        &entry,
                        candidates.len() * std::mem::size_of::<LeanImpostorCandidate>(),
                    );
                    candidates
                });
    ...
    /// Cache misses evaluate disjoint fixed regions once; hits only filter the
    /// retained tiers for this stride. The final sort restores the original
    /// ci-major/cj-minor enumeration order, making cache state unobservable to
    /// vertex emission and settled captures.
    pub(crate) fn impostor_candidates(
        ...
        debug_assert!(face < 6);
        debug_assert!(ci_end < COLUMNS_PER_FACE && cj_end < COLUMNS_PER_FACE);
        debug_assert!(IMPOSTOR_CANDIDATE_STRIDES.contains(&stride));
        ...
        let mut keys = Vec::with_capacity(
            (usize::from(ri1 - ri0) + 1) * (usize::from(rj1 - rj0) + 1),
        );
        for ri in ri0..=ri1 {
            for rj in rj0..=rj1 {
                keys.push(ImpostorCandidateRegionKey { face, ri, rj });
            }
        }

        let comp = (stride * stride) as f64;
        let mut candidates: Vec<ImpostorCandidate> = keys
            .iter()
            .flat_map(|&key| {
                ...
                for tier in self.impostor_candidate_region_tiers(key, stride) {
                    for candidate in tier.iter().copied() {
                        let ci = region_ci0 + u64::from(candidate.ci_offset);
                        let cj = region_cj0 + u64::from(candidate.cj_offset);
                        if ci < ci_start || ci > ci_end || cj < cj_start || cj > cj_end {
                            continue;
                        }
                        let lot = crate::voxel::tree_hash01(face, ci, cj, self.seed);
                        if lot >= crate::voxel::MAX_TREE_DENSITY * comp
                            || lot >= candidate.density * comp
                        {
                            continue;
                        }
                        local.push((ci, cj, candidate.kind, lot, candidate.density));
                    }
                }
                local
            })
...
mod tests {
    ...
    fn impostor_candidate_cache_matches_uncached_stream_across_strides() {
        ...
        let expected: Vec<Vec<ImpostorCandidate>> = strides
            .iter()
            .map(|&stride| {
                uncached_impostor_candidates(&planet, 0, ci0, ci1, cj0, cj1, stride)
            })
            .collect();
        ...
        // A fresh cache may receive overlapping tier misses from different
        // Rayon workers. OnceLock serializes only each shared region/tier;
        // every caller still observes the same ordered stream.
        use rayon::prelude::*;
    ...
    fn impostor_whole_tile_proof_combines_phase_and_late_rejections() {
        ...
        for face in &mut planet.faces {
            for y in 0..res {
                for x in 0..res {
                    ...
                    face.climate[y * res + x][0] =
                        if x < res / 2 - 12 { 8.0 } else { -20.0 };
                }
            }
        }
        let to_col = |uv: f64| {
        ...
        assert!(!planet.impostor_candidate_bounds_phase_reject_all(
            0, ci0, ci1, cj0, cj1,
        ));
    ...
    fn impostor_candidate_cache_enforces_entry_and_byte_bounds() {
        ...
        let heavy_key = (0..u16::MAX)
            .flat_map(|ri| (0..32).map(move |rj| ImpostorCandidateRegionKey {
                face: 1,
                ri,
                rj,
            }))
